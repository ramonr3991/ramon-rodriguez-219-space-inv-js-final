...
var invaderSpriteHeightsArray = [[68,102],[102,134],[102,34],[0,34],[0,34]];
...
function invadersBulletHandler(){
    if(invaderBulletsArray.length<3 && frameCount- invBullet_prevFrameCount>(armySpeed*armyInvaderBulletSpeed)){
...
function moveInvaderBullets(){
    for(let i = 0 ; i < invaderBulletsArray.length; i++){
        ...
        if(
            ...
            iB.y > tankY &&
            iB.y < tankY + tankWidth 
        )
        {
            explosionColor = "green";
            particlesPerExplosion = 450;
            particlesMaxSize = 8;
...
function helperHandler(){
    if(aliveInvaders == armyColumns*armyRows) {
        drawBottomMessage("press SPACE to release xenon laser", 125);
    } else
    if(hasLifeDecreased){
...
function fireTankBullet(){
    ...
    tankBullet__y = canvas.height - tank_bottomOffset;
    drawBullet(tankBullet__x, tankBullet__y);
    moveTankBullet();
...
function moveTankBullet(){
    ...
    //check if invader is zapped by laser
    for (let i = 0; i <  armyRows; i++) {
        for(let j = 0; j < armyColumns; j++) {
            let soldier = armyArray[i][j];
            if(
...
function drawBottomHelper(){
    drawBottomMessage("Press enter to start mission", 150)
}
...
function drawInvaderBullet(ix, iy){
    ctx.beginPath();
    ctx.beginPath();   //Start a new path
    ctx.moveTo(ix, iy); //Move the pen from (30,50)
    ctx.lineTo(ix, iy + bullet_height); //draw a line to (100,150)
...
function drawGameOver(message){
    drawBlinker(function(){ drawScreen__line1("MISSION FAILED") }, function(){ drawScreen__line2("Ohhhhhh SpongeBob....whhhhyyyy?????") });
}
...
function drawStartScreen(){
    drawBlinker(function(){ drawScreen__line1("SPACE INVADERS") }, function(){drawScreen__line2("Press enter to start mission") });
}
